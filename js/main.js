document.getElementById('bub1').onclick = function(event) {
    document.getElementById('bub1').style.display = "none";
}

document.getElementById('bub2').onclick = function(event) {
    document.getElementById('bub2').style.display = "none";
}

document.getElementById('bub3').onclick = function(event) {
    document.getElementById('bub3').style.display = "none";
}

document.getElementById('bub4').onclick = function(event) {
    document.getElementById('bub4').style.display = "none";
}

document.getElementById('bub5').onclick = function(event) {
    document.getElementById('bub5').style.display = "none";
}

document.getElementById('bub6').onclick = function(event) {
    document.getElementById('bub6').style.display = "none";
}

document.getElementById('bub7').onclick = function(event) {
    document.getElementById('bub7').style.display = "none";
}

document.getElementById('bub8').onclick = function(event) {
    document.getElementById('bub8').style.display = "none";
}

document.getElementById('bub9').onclick = function(event) {
    document.getElementById('bub9').style.display = "none";
}

document.getElementById('bub10').onclick = function(event) {
    document.getElementById('bub10').style.display = "none";
}

document.getElementById('bub11').onclick = function(event) {
    document.getElementById('bub11').style.display = "none";
}

document.getElementById('bub12').onclick = function(event) {
    document.getElementById('bub12').style.display = "none";
}

document.getElementById('bub13').onclick = function(event) {
    document.getElementById('bub13').style.display = "none";
}

document.getElementById('bub14').onclick = function(event) {
    document.getElementById('bub14').style.display = "none";
}

document.getElementById('bub15').onclick = function(event) {
    document.getElementById('bub15').style.display = "none";
}

document.getElementById('bub16').onclick = function(event) {
    document.getElementById('bub16').style.display = "none";
}

document.getElementById('bub17').onclick = function(event) {
    document.getElementById('bub17').style.display = "none";
}

document.getElementById('bub18').onclick = function(event) {
    document.getElementById('bub18').style.display = "none";
}

document.getElementById('bub19').onclick = function(event) {
    document.getElementById('bub19').style.display = "none";
}

document.getElementById('bub20').onclick = function(event) {
    document.getElementById('bub20').style.display = "none";
}